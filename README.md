...
- **Reasoning effort was dropped rather than ported.** Upstream ran `review-work`'s reviewers at `xhigh`; the `Agent` tool takes a `model` tier alias and has no effort parameter, so the roster now names tiers only. Only `Workflow`'s `agent()` accepts `effort` — moving the fan-out there is what it would take to get that control back.
