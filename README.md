...
| [`manual-qa-plan/`](manual-qa-plan/SKILL.md) | Writes the QA hand-off document for a `<BASE>..HEAD` range — a durable file built from `assets/qa-plan-template.md`, not a chat answer — that a tester executes against the running application without reading source, recording the outcome per case ID. Every changed file must be accounted for — a test case, an explicit "not user-visible" row with a reason, or an open question — and every changed behaviour records both the old and the new side. Ships two scripts: `collect_changes.sh` pre-groups the diff into risk categories and flags what no heuristic matched, and `verify_coverage.sh` proves mechanically that no changed path is missing from the finished document before it is handed over. | Original |
