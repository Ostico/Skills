...
- **`review-work` still dumps `FILE_CONTENTS` into every prompt.** Upstream's Oracle was a tool-less reasoning agent that could not read files, so the skill pastes the full text of every changed file into all five prompts. Claude Code subagents have full tool access, making that unnecessary and expensive — it burns the orchestrator's context re-transmitting files the subagent could open itself. The compatibility table at the top of the file overrides the behaviour, but the prose underneath it has not been rewritten. This is a design change, not a substitution, which is why it was left.
...
- **Reasoning effort is not expressible through the `Agent` tool.** `review-work`'s roster asks for `Opus xhigh` and `Sonnet xhigh`; the tool takes a `model` tier alias and has no effort parameter. Only `Workflow`'s `agent()` accepts `effort`, so the fan-out has to move to `Workflow` for that to mean anything.
