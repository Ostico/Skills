...
- `review-work/SKILL.md` still carries upstream OpenCode/oh-my-openagent tool syntax — `task(subagent_type="oracle", category="unspecified-high", load_skills=[...])` and `background_output(task_id=...)`. On Claude Code the equivalent is `Agent(subagent_type=...)`, with completion delivered as an automatic notification — note that there is no `run_in_background` parameter on the `Agent` tool, because subagents always run in the background. It also has no YAML frontmatter, so discovery depends on the installed directory name. Porting it the way `parallel-planning` was ported (compatibility table at the top, native tool names) is the outstanding work.
