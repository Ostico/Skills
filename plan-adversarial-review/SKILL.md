...
- If the Workflow tool is unavailable (no multi-agent opt-in), dispatch three parallel subagents via the Agent tool with the same three lens prompts, then synthesize their reports yourself. Pass `model="opus"` on each: a subagent cannot inherit a `[1m]` session model and the call is denied without an explicit tier alias. Do not pass `name`, or the reports will not come back to you. Note in the verdict that `effort` was unavailable on this path.
